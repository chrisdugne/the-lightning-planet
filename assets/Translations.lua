local translations =
{
    ["Full version"] = {
        ["fr"] = "Version complète",
    },

    ["- Play all Levels\n- No more time limit"] = {
        ["fr"] = "- Tous les niveaux\n- Aucune limite de temps ",
    },

    ["Cancel"] = {
        ["fr"] = "Annuler",
    },

    ["Reset the game"] = {
        ["fr"] = "Remise à 0 du jeu",
    },

    ["Confirm now to erase your level progression and start the game again"] = {
        ["fr"] = "Confirmer maintenant effacera votre progression et recommencera le jeu à 0",
    },

    ["Fail !"] = {
        ["fr"] = "Raté !",
    },

    ["Time Attack"] = {
        ["fr"] = "Time Attack",
    },

    ["Classic"] = {
        ["fr"] = "Classique",
    },

    ["Combo"] = {
        ["fr"] = "Combo",
    },

    ["Kamikaze"] = {
        ["fr"] = "Kamikaze",
    },

    ["Buy"] = {
        ["fr"] = "Acheter",
    },

    ["The game is locked\n Buy me a coffee to get access to the full game !"] = {
        ["fr"] = "Le jeu est en démo\n Achetez moi un café pour accéder à son intégralité !",
    },

    ["Tutorial"] = {
        ["fr"] = "Tutoriel",
    },
    
    ["Easy"] = {
        ["fr"] = "Facile",
    },
    
    ["Hard"] = {
        ["fr"] = "Difficile",
    },
    
    ["Extreme"] = {
        ["fr"] = "Extrême",
    },
    
    ["Here are counted caught asteroids"] = {
        ["fr"] = "On compte les astéroïdes attrapés",
    },
  
    ["The more asteroids you gather..."] = {
        ["fr"] = "Plus vous attrapez d'astéroïdes...",
    },
  
    ["...the more points the next one will give"] = {
        ["fr"] = "...plus la suivante vaudra de points",
    },
  
    ["This is the planet's Energy"] = {
        ["fr"] = "Voici l'Energie de la planète",
    },
  
    ["Catching a wrong color decrease the count"] = {
        ["fr"] = "attraper une mauvaise couleur diminue le compte",
    },
  
    ["Cost Energy"] = {
        ["fr"] = "Coûte de l'Energie",
    },
  
    ["And decrease points"] = {
        ["fr"] = "Et baisse les points",
    },
  
    ["A blue asteroid is coming"] = {
        ["fr"] = "Un astéroïde bleu arrive",
    },
  
    ["A green asteroid is coming"] = {
        ["fr"] = "Un astéroïde vert arrive",
    },
  
    ["It's the same color, you have to destroy this asteroid"] = {
        ["fr"] = "C'est la même couleur, il faut détruire cet astéroïde",
    },
    
    ["Call Lightning  !"] = {
        ["fr"] = "Invoquez l'Eclair !",
    },
  
    ["Lightning destroys the closest asteroid"] = {
        ["fr"] = "L'Eclair détruit l'astéroïde le plus proche",
    },
  
    ["Here is the Combo : you have to destroy asteroids in that order before they crash on the planet"] = {
        ["fr"] = "Voici la Combo à réaliser : il faut détruire les astéroides dans l'ordre avant qu'ils ne touchent la planète",
    },

    ["Now we have to wait for a green asteroid to destroy"] = {
        ["fr"] = "Maintenant il faut attendre un astéroide vert",
    },

    ["This one is not green, and if you use the Lightning on a wrong color, your start the combo from scratch"] = {
        ["fr"] = "Celui ci n'est pas vert, et si vous utilisez l'Eclair sur une mauvaise couleur, vous recommencez la Combo à 0",
    },

    ["So you have to catch it with the Planet. But remember : you must have the same planet color when the asteroid crashes, or Game Over !"] = {
        ["fr"] = "Mais n'oubliez pas : la planète doit être de la même couleur lorsque l'astéroïde s'écrase, sinon c'est perdu !",
    },
  
    ["Lightning squares the color count as points !"] = {
        ["fr"] = "L'Eclair transforme le compte au carré en points",
    },
  
    ["And reduces the count"] = {
        ["fr"] = "Et réduit le compte",
    },
  
    ["Get the more points before the end of Energy !"] = {
        ["fr"] = "Obtenez le plus de points avant la fin de l'Energie !",
    },

    ["Get the more points before the end of Time !"] = {
        ["fr"] = "Obtenez le plus de points avant la fin du Temps !",
    },
  
    ["Tutorial Complete !"] = {
        ["fr"] = "Tutoriel Terminé !",
    },
  
    ["This is your Planet"] = {
        ["fr"] = "Voici votre Planète",
    },
  
    ["And this is an asteroid"] = {
        ["fr"] = "Et voilà un astéroïde",
    },
  
    ["These buttons change the planet's color"] = {
        ["fr"] = "Ces boutons changent la couleur de la Planète",
    },
  
    ["Turn your planet green"] = {
        ["fr"] = "Changez la planète en vert",
    },
  
    ["You catch an asteroid when it's the same color"] = {
        ["fr"] = "Les astéroïdes doivent toucher la planète en ayant la même couleur",
    },
  
    ["Lightning destroys the closest asteroid !"] = {
        ["fr"] = "L'Eclair détruit l'astéroïde le plus proche",
    },
  
    ["To achieve a Combo, you have to destroy asteroids"] = {
        ["fr"] = "Pour réussir une Combo, il faut détruire les astéroïdes",
    },
  
    ["Here is the color to destroy"] = {
        ["fr"] = "Voici la couleur à détruire",
    },
  
    ["Here are the next colors to destroy to achieve the Combo"] = {
        ["fr"] = "Et voici les couleurs suivantes qu'il faudra détruire pour réussir la Combo",
    },
  
    ["Turn your planet blue"] = {
        ["fr"] = "Changez la planète en bleu",
    },
  
    ["Planet and asteroid have the same color : Well done !"] = {
        ["fr"] = "Planète et astéroïde ont bien la même couleur maintenant",
    },
  
    ["Now you have to catch a green asteroid"] = {
        ["fr"] = "Ensuite il nous faut un astéroïde vert",
    },
  
    ["Great !"] = {
        ["fr"] = "Bien !",
    },
  
    ["Finally you need a blue asteroid"] = {
        ["fr"] = "Enfin,  il nous faut un astéroïde bleu",
    },
  
    ["Wait ! This one is green !"] = {
        ["fr"] = "Attention ! Celle ci est verte !",
    },
  
    ["If you catch it now you miss the combo"] = {
        ["fr"] = "En l'attrapant maintenant, on va rater la Combo",
    },
  
    ["And if you turn the planet blue, you miss the catch"] = {
        ["fr"] = "Et si on change la planète en bleu, elle va s'écraser",
    },
  
    ["and fail the combo as well"] = {
        ["fr"] = "et manquer la Combo aussi",
    },
  
    ["You need the Lightning !"] = {
        ["fr"] = "Il nous faut l'Eclair",
    },
  
    ["Now this green asteroid is no more"] = {
        ["fr"] = "Maintenant l'astéroïde vert ne gêne plus",
    },
  
    ["Here is the final blue asteroid !"] = {
        ["fr"] = "Voilà le dernier astéroïde bleu !",
    },
  
    ["Nice !"] = {
        ["fr"] = "Bien joué !",
    },
  
    ["Well done ! Now you're ready to play !"] = {
        ["fr"] = "Et voilà, vous êtes prêt à jouer !",
    },

    ["Reach 2 min to unlock Combo mode"] = {
        ["fr"] = "Atteignez 2 min pour débloquer le mode Combo",
    },
  
}


return translations